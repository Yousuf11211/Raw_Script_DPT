...
# - Standardized outputs under ./outputs/Model_XGBoost with final summary.
#
# Purpose:
# - Train XGBoost models from CSV files.
...
MODEL_FOLDER = os.path.join(OUTPUT_ROOT, "Model_XGBoost", "models")
REPORT_FOLDER = os.path.join(OUTPUT_ROOT, "Model_XGBoost", "reports")
...
def fit_xgb_model(X, y, device):
    ...
    try:
        ...
        if device == "gpu":
            ...
            model = xgb.XGBClassifier(**build_xgb_params("cpu"))
            model.fit(X, y)
            return model, "cpu"
        raise
...
def train_xgboost_from_csv(
    ...
    label_mapping_filename: str = "xgboost_label_mapping.txt",
    device: str = "cpu",
    ...
    xgb_model, _ = fit_xgb_model(X, y, device)
...
def process_csv(file_path, device, chunk_rows):
    print(f"\n{'=' * 80}")
    ...
    if train_full_data:
        print("Training XGBoost on sampled dataset...")
        xgb_model, device_used = fit_xgb_model(X, y, device)
        joblib.dump(xgb_model, model_path)
        ...
        xgb_model, device_used = fit_xgb_model(X_train, y_train, device)
        joblib.dump(xgb_model, model_path)
        print(f"Model trained and saved to {model_path}")
...
if __name__ == "__main__":
    ...
    if not csv_files:
        ...
        for file_path in csv_files:
            rows_processed, _, paths = process_csv(file_path, device_choice, chunk_rows)
        ...
        print("\nFinal Summary")
        print("-" * 40)
        print(f"Device used: {device_choice.upper()}")
